...
Read [exercise models](exercise-models.md) when designing a substantial problem set. It separates Griffiths's placement of immediate and end-of-chapter problems from the linked derivation, boundary-case, and implementation tasks in the supplied textbook.
...
Check that the task has sufficient information and that its solution answers every part. Allow multiple valid methods, parameter-dependent conclusions, or a proof of non-uniqueness when appropriate. For an engineering design comparison, state the constraints and evaluation criteria. For a numerical problem, specify relevant units and tolerances.

If assessment is requested, align the marking criteria with the learning goal. A conceptual argument should not be graded mainly on formatting. Provide partial-credit guidance for substantive intermediate reasoning, and separate an instructor's rubric from the learner's version when appropriate.
