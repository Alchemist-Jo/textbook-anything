...
Each round includes all of the following within the requested scope:

1. Revisit the requirement table, learner assumptions, source coverage, and unresolved findings.
2. Work through the complete tutorial: fill or revise foundations, inherited methods, focal explanation, examples, visuals, exercises, and answers as applicable.
3. Produce the actual deliverable, including a built PDF or working HTML and its local assets.
4. Inspect the whole tutorial for learning sequence, reasoning, language, figure semantics, and layout. Solve or independently check the exercises and run applicable code or numerical checks.
5. Record concrete findings, correct them, rebuild the affected output, and recheck what changed.
6. Update the requirement table and the short [round record](../templates/round-record.md) with actual results and remaining work.

A round may reuse valid sources and checks from unaffected material, but it still reviews the whole agreed learning sequence. Two compiler passes, two lint runs, or a second read of the introduction are not separate complete rounds.

## Round focus

| Round | Main question | Work to emphasize |
| --- | --- | --- |
...
For each requirement, state whether it is complete, needs revision, or is blocked by a specific missing capability or source. Record the actual location, the finding, the correction, and the recheck. A clean section can be recorded as checked with no material finding; do not invent an edit to make the loop appear productive.
