...
进入一个新领域时，困难往往在于还不知道自己缺了哪一部分。可能看得懂代码，却不清楚目标函数的假设；也可能会做计算，但不知道领域里为什么要研究这个问题。

textbook-anything 会先选定一个具体的入门目标，再判断哪些缺口妨碍理解：领域问题与术语、数学表示、操作步骤、默认约定，或对实验结果的判断。必要的背景会在使用它之前讲清楚，随后安排示范、引导练习和独立变式，让读者有机会检查自己是否真的理解。

“从零开始”会尊重读者已经掌握的知识。没有做过的检查会保留为尚未评估，不会因为一份讲义生成完成，就记录成读者已经学会。可以先看[从一阶系统入门的例子](examples/field-entry/README.md)。

## 按模型额度选择档位

| 档位 | 适合的目标 | 工作范围 | 完整轮次 |
| --- | --- | --- | --- |
| 精简 | 先解决一个明确的入门问题 | 必要先修、一条完整讲解路径、例题、独立练习与解答 | 1 |
| 标准，默认 | 学完一个连贯的专题或章节 | 主要背景与方法、相关比较、递进习题与迁移检查 | 2 |
| 深入 | 系统进入新领域或深入学习复杂论文 | 多种方法及其联系、争议与边界、数学和实践的多角度解释 | 3 |

档位控制研究广度和处理轮次。每档都保留必要的推导、图示、答案和成品检查，实际消耗随材料复杂度与使用环境变化。模型额度与学习时间分开考虑；使用 GPT Pro 也可以选择精简档。

如果范围很大而额度有限，会先把它安排成几个可独立学习的单元，明确本次完成什么、哪些内容后续继续。升级档位时沿用已有讲解、来源和真实练习记录。[查看档位说明](references/resource-tiers.md)。
...
全文按学习目标和实际知识缺口分配篇幅，再按所选档位完成编写、成品检查与修订。每一轮都检查基础知识、核心方法、图示与习题，并修正发现的问题。
...
直接说明主题、读者、额度档位和想要的产物。未指定档位时使用标准档：

```text
使用 textbook-anything，标准档，为工科大二学生编写傅里叶分析一章。
...
```text
使用 textbook-anything，精简档。我会基础微积分，刚开始接触动力系统。
先讲清状态、参数和初始条件的区别，用一个完整例子解释一阶衰减模型，
再给一道我能独立尝试的变式题和解答。数值方法留到下一单元。
```
...
目前是 v0.2，欢迎从适合自己额度和学习目标的档位开始试用。
